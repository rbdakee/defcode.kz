...
/**
 * Заголовок стоит в общей сетке, а лента идёт от края до края экрана:
 * ей нужна вся ширина, иначе на отступах видно, как список «обрывается».
 * Края гасит маска — раньше эту работу делало скругление тёмной панели.
 */
      ...
      >
        <ul className="marquee-track flex w-max gap-3">
          {[...stack, ...stack].map((tech, i) => (
            <li
              key={`${tech}-${i}`}
              className="flex items-center gap-2.5 rounded-full bg-paper px-4 py-2 font-mono text-sm whitespace-nowrap text-muted"
            >
              <TechLogo name={tech} className="size-4 text-brand" />
              {tech}
            </li>
          ))}
        </ul>
