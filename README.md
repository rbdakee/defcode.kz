...
**Фон белый везде, «пузыри» — только карточки.** Подложек во всю ширину нет:
`Section` в [src/components/ui.tsx](src/components/ui.tsx) задаёт лишь
вертикальный ритм, заголовок и подзаголовок секции стоят прямо на белом.
Пузырь — это карточка внутри: константы `bubble` и `bubbleLink` там же,
заливка `paper` без рамки. Рамок и разделительных линий избегаем намеренно —
границу держит цвет, линия дала бы ту же границу дважды (поэтому шаги
процесса и вопросы FAQ тоже карточки, а не строки с `border-t`).
Цветной пузырь на сайте один — полоса призыва в конце страницы.
Секция стека собрана вручную: заголовок в общей сетке, а бегущая строка идёт
во всю ширину экрана, её края гасит маска.
...
градиенты, не размытые круги: у градиента прозрачный край, поэтому он гаснет
сам. Обрезать его нельзя: `overflow-hidden` на секции даёт ровный срез, и на
белом фоне пятно читается прямоугольной заплаткой. Горизонтальный сдвиг
от пятен снимает `overflow-x: hidden` у `body`.
